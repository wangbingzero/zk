...
public class ZooKeeperServerMain {
    ...
    public void runFromConfig(ServerConfig config) throws IOException, AdminServerException {
        ...
        try {
            ...
            //创建服务器统计器ServerStats,根据config设置相关参数
            final ZooKeeperServer zkServer = new ZooKeeperServer(jvmPauseMonitor,
                    txnLog, config.tickTime, config.minSessionTimeout,
                    config.maxSessionTimeout, config.listenBacklog, null,
...
//                通过配置系统属性zookeper.serverCnxnFactory来指定使用Zookeeper自己实现的
//                NIO还是使用Netty框架作为Zookeeper服务端网络连接工厂。
                //默认使用NIO的
                cnxnFactory = ServerCnxnFactory.createFactory();
                //初始化主线程，打开selector，并且bind端口，打开accept通知
                cnxnFactory.configure(config.getClientPortAddress(), config.getMaxClientCnxns(), config.getClientPortListenBacklog(), false);
                //生成最新的snapshot文件，启动IO线程，从snapshot文件和log文件中恢复内存database结果
